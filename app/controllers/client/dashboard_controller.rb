class Client::DashboardController < ClientController
  ...
  def dashboard
    ...
    venue_points = Venue.joins(:tours).where.not(lat: nil, lng: nil).closest(origin: [cookies[:lat],cookies[:lng]]).uniq.limit(2)
    # venue_points = Venue.joins(:tours).where.not(lat: nil, lng: nil).closest(origin: [cookies[:lat],cookies[:lng]]).uniq.limit(2)
    @venues = Api::V1::Parser.venue_parser(venue_points).flatten
  end
